...
namespace CutTheRope.GameMain
{
    internal sealed partial class GameScene
    {
        ...
        {
            if (PointInRect(tx + camera.pos.XAxis, ty + camera.pos.YAxis, s.pos.XAxis - 60f, s.pos.YAxis - 60f, 120f, 120f))
            {
        ...
        private bool HandleLightBulbBubbleTouch(LightBulb bulb, float tx, float ty)
        {
            ...
            if (PointInRect(tx + camera.pos.XAxis, ty + camera.pos.YAxis, bulb.constraint.pos.XAxis - BUBBLE_RADIUS, bulb.constraint.pos.YAxis - BUBBLE_RADIUS, BUBBLE_RADIUS * 2f, BUBBLE_RADIUS * 2f))
            {
        ...
        public bool TouchDownXYIndex(float tx, float ty, int ti)
        {
            ...
            if (gravityButton != null && ((Button)gravityButton.GetChild(gravityButton.On() ? 1 : 0)).IsInTouchZoneXYforTouchDown(tx + camera.pos.XAxis, ty + camera.pos.YAxis, true))
            {
                gravityTouchDown = ti;
            }
            float worldX = tx + camera.pos.XAxis;
            float worldY = ty + camera.pos.YAxis;
            ...
            foreach (object obj in spikes)
            {
                ...
                if (spike.rotateButton != null && spike.touchIndex == -1 && spike.rotateButton.OnTouchDownXY(tx + camera.pos.XAxis, ty + camera.pos.YAxis))
                {
                    spike.touchIndex = ti;
                    return true;
                }
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                if (GameObject.PointInObject(Vect(tx + camera.pos.XAxis, ty + camera.pos.YAxis), pump))
                {
            ...
            {
                if (steamTube != null && steamTube.OnTouchDownXY(tx + camera.pos.XAxis, ty + camera.pos.YAxis))
                {
                    return true;
                }
            }
            ...
            {
                if (lantern != null && lantern.OnTouchDown(tx + camera.pos.XAxis, ty + camera.pos.YAxis))
                {
            ...
            foreach (object obj2 in rotatedCircles)
            {
                ...
                float num2 = VectDistance(Vect(tx + camera.pos.XAxis, ty + camera.pos.YAxis), rotatedCircle2.handle1);
                float num3 = VectDistance(Vect(tx + camera.pos.XAxis, ty + camera.pos.YAxis), rotatedCircle2.handle2);
                if ((num2 < 90f && !rotatedCircle2.HasOneHandle()) || num3 < 90f)
                {
                    ...
                    rotatedCircle2.lastTouch = Vect(tx + camera.pos.XAxis, ty + camera.pos.YAxis);
            ...
            if (ghosts != null)
            {
                foreach (object objGhost in ghosts)
                {
                    ...
                    if (ghost != null && ghost.OnTouchDownXY(tx + camera.pos.XAxis, ty + camera.pos.YAxis))
                    {
                        return true;
                    }
                }
            ...
            foreach (object obj4 in bungees)
            {
                ...
                if (bungee.wheel && PointInRect(tx + camera.pos.XAxis, ty + camera.pos.YAxis, bungee.x - 110f, bungee.y - 110f, 220f, 220f))
                {
                    bungee.HandleWheelTouch(Vect(tx + camera.pos.XAxis, ty + camera.pos.YAxis));
                    bungee.wheelOperating = ti;
                }
                if (bungee.moveLength > 0.0 && PointInRect(tx + camera.pos.XAxis, ty + camera.pos.YAxis, bungee.x - 65f, bungee.y - 65f, 130f, 130f))
                {
            ...
            if (clickToCut && !ignoreTouches)
            {
                Vector s = default;
                Grab grab2 = null;
                Bungee nearestBungeeSegmentByBeziersPointsatXYgrab = GetNearestBungeeSegmentByBeziersPointsatXYgrab(ref s, tx + camera.pos.XAxis, ty + camera.pos.YAxis, ref grab2);
        ...
        public bool TouchUpXYIndex(float tx, float ty, int ti)
        {
            ...
            {
                if (((Button)gravityButton.GetChild(gravityButton.On() ? 1 : 0)).IsInTouchZoneXYforTouchDown(tx + camera.pos.XAxis, ty + camera.pos.YAxis, true))
                {
            ...
            foreach (object obj in spikes)
            {
                ...
                {
                    spike.touchIndex = -1;
                    if (spike.rotateButton.OnTouchUpXY(tx + camera.pos.XAxis, ty + camera.pos.YAxis))
                    {
                        return true;
                    }
                }
            ...
            _ = conveyors.OnPointerUp(tx + camera.pos.XAxis, ty + camera.pos.YAxis, ti);
            return true;
        }
        ...
        public bool TouchMoveXYIndex(float tx, float ty, int ti)
        {
            ...
            if (rotatedCircles != null)
            {
                for (int i = 0; i < rotatedCircles.Count; i++)
                {
                    ...
                    if (rotatedCircle != null && rotatedCircle.operating == ti)
                    {
                        ...
                        Vector vector2 = Vect(tx + camera.pos.XAxis, ty + camera.pos.YAxis);
                        ...
                        for (int j = 0; j < bungees.Count; j++)
                        {
                            ...
                            if (VectDistance(Vect(grab.x, grab.y), Vect(rotatedCircle.x, rotatedCircle.y)) <= rotatedCircle.sizeInPixels + 5f)
                            {
                                ...
                                grab.x = vector3.XAxis;
                                grab.y = vector3.YAxis;
                                if (grab.rope != null)
                                {
                        ...
                        for (int k = 0; k < pumps.Count; k++)
                        {
                            ...
                            if (VectDistance(Vect(pump4.x, pump4.y), Vect(rotatedCircle.x, rotatedCircle.y)) <= rotatedCircle.sizeInPixels + 5f)
                            {
                                ...
                                pump4.x = vector4.XAxis;
                                pump4.y = vector4.YAxis;
                        ...
                        for (int l = 0; l < bubbles.Count; l++)
                        {
                            ...
                            if (VectDistance(Vect(bubble.x, bubble.y), Vect(rotatedCircle.x, rotatedCircle.y)) <= rotatedCircle.sizeInPixels + 10f && bubble != candyBubble && bubble != candyBubbleR && bubble != candyBubbleL)
                            {
                                ...
                                bubble.x = vector5.XAxis;
                                bubble.y = vector5.YAxis;
                            }
                        }
                        if (PointInRect(target.x, target.y, rotatedCircle.x - rotatedCircle.size, rotatedCircle.y - rotatedCircle.size, 2f * rotatedCircle.size, 2f * rotatedCircle.size))
                        {
                            ...
                            target.x = vector6.XAxis;
                            target.y = vector6.YAxis;
                        }
            ...
            for (int m = 0; m < num3; m++)
            {
                ...
                if (grab2 != null)
                {
                    ...
                    {
                        grab2.HandleWheelRotate(Vect(tx + camera.pos.XAxis, ty + camera.pos.YAxis));
                        return true;
                    }
                    if (grab2.moveLength > 0.0 && grab2.moverDragging == ti)
                    {
                        ...
                        {
                            grab2.y = FIT_TO_BOUNDARIES(ty + camera.pos.YAxis, grab2.minMoveValue, grab2.maxMoveValue);
                        }
                        else
                        {
                            grab2.x = FIT_TO_BOUNDARIES(tx + camera.pos.XAxis, grab2.minMoveValue, grab2.maxMoveValue);
                        }
                        if (grab2.rope != null)
                        {
            ...
            if (conveyors.OnPointerMove(tx + camera.pos.XAxis, ty + camera.pos.YAxis, ti))
            {
                return true;
            }
            if (dragging[ti])
