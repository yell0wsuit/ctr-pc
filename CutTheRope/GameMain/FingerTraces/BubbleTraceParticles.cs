...
{
    /// <summary>
    /// Lightweight particle emitter used by <see cref="BubbleFingerTrace"/> for the CTR2 bubble trail.
    /// </summary>
    internal sealed class BubbleTraceParticles : FrameworkTypes
    {
        ...
        public void Update(float delta)
        {
            ...
            {
                BubbleParticle particle = particles[i];
                particle.Life -= delta;
                if (particle.Life <= 0f)
                {
                ...
                if (distance > 0.0001f)
                {
                    ...
                    Vector accel = VectMult(radialDir, RadialAcceleration);
                    particle.Velocity = VectAdd(particle.Velocity, VectMult(accel, delta));
                }
        ...
        private BubbleParticle CreateParticle()
        {
            ...
            float startScale = 1.0f + (0.3f * RND_MINUS1_1);
            float endSpinDeg = 180f * RND_MINUS1_1;
            Vector spawnPos = new(
