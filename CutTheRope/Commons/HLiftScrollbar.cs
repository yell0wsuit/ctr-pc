...
namespace CutTheRope.Commons
{
    internal sealed class HLiftScrollbar : Image
    {
        ...
        public void UpdateActiveSpoint()
        {
            ...
            while (i < spointsNum)
            {
                if (lift.x <= spointsLimits[i].XAxis)
                {
        ...
        public override void Update(float delta)
        {
            ...
            if (lift.x >= spointsLimits[spointsNum - 1].XAxis && activeSpoint != limitPoints[spointsNum - 1])
            {
        ...
        public void PercentXY(float px, float py)
        {
            ...
            container.SetScroll(Vect(maxScroll.XAxis * px, maxScroll.YAxis * py));
        }
        ...
        public void UpdateLift()
        {
            ...
            Vector maxScroll = container.GetMaxScroll();
            float num = 0f;
            if (maxScroll.XAxis != 0f)
            {
                num = scroll.XAxis / maxScroll.XAxis;
            }
            if (maxScroll.YAxis != 0f)
            {
                _ = scroll.YAxis / maxScroll.YAxis;
            }
        ...
        public void CalcScrollPoints()
        {
            ...
            for (int i = 0; i < spointsNum; i++)
            {
                ...
                if (maxScroll.XAxis != 0f)
                {
                    num = vector.XAxis / maxScroll.XAxis;
                }
                if (maxScroll.YAxis != 0f)
                {
                    _ = vector.YAxis / maxScroll.YAxis;
                }
            ...
            while (flag)
            {
                ...
                {
                    if (spointsLimits[k].XAxis > spointsLimits[k + 1].XAxis)
                    {
                        flag = true;
            ...
            for (int l = 0; l < spointsNum - 1; l++)
            {
                ...
                int num6 = l;
                array[num6].XAxis = array[num6].XAxis + ((vector4.XAxis - vector3.XAxis) / 2f);
            }
        }
