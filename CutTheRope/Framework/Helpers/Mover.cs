    ...
    internal class Mover : FrameworkTypes
    {
        ...
        public virtual void SetPathFromStringandStart(string p, Vector s)
        {
            if (p.CharacterAtIndex(0) == 'R')
            {
                ...
                {
                    float x = s.XAxis + (num * (float)Math.Cos((double)num4));
                    float y = s.YAxis + (num * (float)Math.Sin((double)num4));
            ...
            for (int j = 0; j < list.Count; j += 2)
            {
                ...
                AddPathPoint(Vect(s.XAxis + nSString2.FloatValue(), s.YAxis + nSString3.FloatValue()));
            }
        }
