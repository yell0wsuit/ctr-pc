...
namespace CutTheRope.Framework.Visual
{
    internal sealed class TileMap : BaseElement
    {
        ...
        {
            float num = (float)Math.Round((double)(pos.XAxis / parallaxRatio));
            float num2 = (float)Math.Round((double)(pos.YAxis / parallaxRatio));
            float num3 = x;
            float num4 = y;
            ...
            Vector vector2 = Vect((int)vector.XAxis / tileWidth, (int)vector.YAxis / tileHeight);
            float num7 = num4 + (vector2.YAxis * tileHeight);
            Vector vector3 = Vect(num3 + (vector2.XAxis * tileWidth), num7);
            int count = drawers.Count;
            for (int i = 0; i < count; i++)
            {
                ImageMultiDrawer imageMultiDrawer = drawers[i];
            ...
            int num8 = (int)(vector2.XAxis + maxColsOnScreen - 1f);
            int num9 = (int)(vector2.YAxis + maxRowsOnScreen - 1f);
            if (repeatedVertically == Repeat.NONE)
            {
            ...
            for (int j = (int)vector2.XAxis; j <= num8; j++)
            {
                vector3.YAxis = num7;
                int k = (int)vector2.YAxis;
                while (k <= num9 && vector3.YAxis < num2 + cameraViewHeight)
                {
                    CTRRectangle rectangle2 = RectInRectIntersection(new CTRRectangle(num, num2, cameraViewWidth, cameraViewHeight), new CTRRectangle(vector3.XAxis, vector3.YAxis, tileWidth, tileHeight));
                    CTRRectangle r = new(num - vector3.XAxis + rectangle2.x, num2 - vector3.YAxis + rectangle2.y, rectangle2.w, rectangle2.h);
                    int num10 = j;
                    int num11 = k;
                    if (repeatedVertically == Repeat.EDGES)
                    {
                        ...
                        else if (vector3.YAxis >= y + tileMapHeight)
                        {
                            num11 = rows - 1;
                        }
                    }
                    if (repeatedHorizontally == Repeat.EDGES)
                    {
                        ...
                        else if (vector3.XAxis >= x + tileMapWidth)
                        {
                            num10 = columns - 1;
                        }
                    }
                    if (horizontalRandom)
                    {
                        num10 = Math.Abs((int)(FmSin(vector3.XAxis) * randomSeed) % columns);
                    }
                    if (verticalRandom)
                    {
                        num11 = Math.Abs((int)(FmSin(vector3.YAxis) * randomSeed) % rows);
                    }
                    if (num10 >= columns)
                    {
                        num10 %= columns;
                    ...
                    if (num12 >= 0)
                    {
                        ...
                        Quad3D qv = Quad3D.MakeQuad3D((double)(pos.XAxis + rectangle2.x), (double)(pos.YAxis + rectangle2.y), 0.0, rectangle2.w, rectangle2.h);
                    ...
                    vector3.YAxis += tileHeight;
                    k++;
                }
                vector3.XAxis += tileWidth;
                if (vector3.XAxis >= num + cameraViewWidth)
                {
                    break;
                }
            }
